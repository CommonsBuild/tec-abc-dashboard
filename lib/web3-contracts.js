...
export async function getNewMintPrice({
  ...
  const newMintPrice = await bondingCurveContract.getStaticPricePPM(
...
export function useGetNewMintPrice(mintedTokens, collateral, bonded) {
  const [newPrice, setNewPrice] = useState(0)
  const bondingCurveContract = useKnownContract('BONDING_CURVE')
  const { reservePoolValue, bondTotalSupply } = useBondingCurvePrice()
  ...
  const fetchNewPrice = async () => {
    ...
    setNewPrice(formatUnits(newMintPrice, { digits: 10 }))
  }
  useEffect(() => {
    fetchNewPrice()
  }, [
