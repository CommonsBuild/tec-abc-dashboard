...
export async function getNewMintPrice({
  // Mint or Burn **
  mintedTokens,
  collateral,
  bonded,
  ...
  const [address, abi] = getKnownContract('BONDING_CURVE')
  const bondingCurveContract = getContract(
    address,
    abi,
    ethersProvider
  )?.connect(ethersProvider.getSigner())

  if (!mintedTokens || !collateral) return
  if (!bondingCurveContract || !bondTotalSupply || !reservePoolValue) return

  const _supply =
    parseFloat(EthersUtils.formatEther(bondTotalSupply.toString())) +
    parseFloat(mintedTokens) * (bonded ? 1 : -1)

  const _balance =
    parseFloat(EthersUtils.formatEther(reservePoolValue.toString())) +
    parseFloat(collateral).toString() * (bonded ? 1 : -1)

  const _reserveRatio = parseInt(reserveRatio / 1e4)
  const newMintPrice = applyTribute(
    await bondingCurveContract.getStaticPricePPM(
      EthersUtils.parseUnits(_supply.toString()),
      EthersUtils.parseUnits(_balance.toString()),
      _reserveRatio
    ),
    bonded ? entryTribute : exitTribute.add(entryTribute)
  )
  return formatUnits(newMintPrice, { digits: 10 })
}

export function useGetNewMintPrice(mintedTokens, collateral) {
  // New mint price **
  ...
  const [entryTribute] = useTributePcts()
  const fetchNewPrice = async () => {
    ...
    setNewPrice(newMintPrice && formatUnits(newMintPrice, { digits: 10 }))
  }
  useEffect(() => {
    fetchNewPrice()
  }, [
